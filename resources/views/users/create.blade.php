...
@section('page-header')
  ...
  <div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
      <div class="d-flex">
        <h4 class="content-title mb-0 my-auto">Users</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ Create
          User</span>
      </div>
    </div>
  </div>
            ...
            <div class=" row row-sm">
              ...
              <div class="col-lg-4">
                <div class="input-group mb-3">
                  ...
                  <input class="form-control @error('password')is-invalid @enderror" type="password" name="password" value="{{ old('password') ?? '' }}" required>
                </div>
              </div>
            </div>
                ...
                <div class="input-group mb-3">
                  ...
                  <select name="role" class="form-control select2-no-search @error('role')is-invalid @enderror" required>
                    ...
                    @foreach ($roles as $role)
                      <option value="{{ $role->id }}" {{ $role->id == old('role') ? 'selected' : '' }}> {{ Str::ucfirst($role->name) }} </option>
                    @endforeach
                  </select>
                </div>
              </div>
